...
import {
  ...
  IconPlus, IconMinus, IconStar, IconChevronRight
...
export function CarDetail({car, onBack, liked, onLike, allReviews, addReview}) {
  ...
  const pills = [
    [<IconSeat  size={13} color="rgba(255,255,255,.7)"/>, `${car.seats} مقاعد`],
    [<IconFuel  size={13} color="rgba(255,255,255,.7)"/>, car.fuel],
    [<IconGear  size={13} color="rgba(255,255,255,.7)"/>, car.trans],
  ];
  ...
  return (
    <div style={{animation:"fadeIn .35s ease", paddingBottom:100}}>
      ...
      {/* ── Spec Pills ── */}
      <div style={{display:"flex", gap:8, flexWrap:"wrap", marginBottom:20, paddingRight:2}}>
        {pills.map(([icon, label], i) => (
          <div key={i} style={{
            ...
            {icon}
            <span style={{fontSize:12, fontWeight:700, color:"rgba(255,255,255,.75)"}}>{label}</span>
          </div>
        ))}
      </div>
