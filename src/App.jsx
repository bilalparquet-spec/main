  ...
  return (
    <div style={{minHeight:"100vh",background:"#07080F",color:"#F1F5F9",overscrollBehavior:"contain"}}>
      ...
      <nav style={{position:"sticky",top:0,zIndex:100,background:"rgba(7,8,15,.92)",backdropFilter:"blur(18px)",borderBottom:"1px solid rgba(255,255,255,.07)",padding:"0 16px",height:58,display:"flex",alignItems:"center",gap:12}}>
        <div onClick={()=>{setPage("home");setSelCar(null);}} style={{display:"flex",alignItems:"center",gap:9,cursor:"pointer",flexShrink:0}}>
          <div style={{width:34,height:34,borderRadius:9,background:"linear-gradient(135deg,#6D28D9,#4F46E5)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 14px rgba(109,40,217,.4)"}}>
            <svg width="19" height="19" viewBox="0 0 100 100" fill="none">
              <rect x="10" y="42" width="80" height="40" rx="14" stroke="white" strokeWidth="11" fill="none"/>
              <rect x="26" y="20" width="48" height="26" rx="11" stroke="white" strokeWidth="11" fill="none"/>
              <circle cx="28" cy="82" r="10" stroke="white" strokeWidth="11" fill="none"/>
              <circle cx="72" cy="82" r="10" stroke="white" strokeWidth="11" fill="none"/>
            </svg>
          </div>
        ...
        {page==="car"&&selCar
          ...
          : <>
                ...
                <div style={{display:"flex",gap:9}}>
                  <div style={{flex:1,position:"relative"}}>
                    <div style={{display:"flex",alignItems:"center",background:"rgba(255,255,255,.06)",border:`1.5px solid ${searchFocus?"rgba(124,58,237,.65)":"rgba(255,255,255,.1)"}`,borderRadius:13,transition:"border-color .22s,box-shadow .22s",boxShadow:searchFocus?"0 0 0 3px rgba(124,58,237,.15)":"none"}}>
                      <span style={{padding:"0 11px",color:searchFocus?"#A78BFA":"rgba(255,255,255,.28)",flexShrink:0}}>
                      ...
                      {searchText&&<button onClick={()=>setSearchText("")} style={{background:"none",border:"none",color:"rgba(255,255,255,.35)",cursor:"pointer",padding:"0 11px",fontSize:17,display:"flex",alignItems:"center",minHeight:44}}>✕</button>}
                    </div>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12,flexWrap:"wrap",gap:6}}>
              ...
              {(nActive>0||searchText)&&<button onClick={()=>{setSearchText("");setAdvF({fuel:"الكل",trans:"الكل",verified:false,minP:"",maxP:""});}} style={{background:"rgba(239,68,68,.08)",border:"1px solid rgba(239,68,68,.2)",color:"#FCA5A5",padding:"3px 11px",borderRadius:20,cursor:"pointer",fontSize:11,fontFamily:"inherit",fontWeight:700}}>مسح ✕</button>}
            </div>

            {/* ══ 4 Sections ══ */}
            <AgenciesSection delay={0}/>
            ...
            {filtered.length===0
              ? <div style={{textAlign:"center",padding:"60px 0",color:"rgba(255,255,255,.3)",animation:"fadeUp .4s ease"}}>
                  ...
                  <button onClick={()=>{setSearchText("");setFType("all");setAdvF({fuel:"الكل",trans:"الكل",verified:false,minP:"",maxP:""});}} style={{marginTop:14,padding:"10px 22px",borderRadius:12,border:"none",background:"linear-gradient(135deg,#6D28D9,#4F46E5)",color:"#fff",fontFamily:"inherit",fontWeight:700,cursor:"pointer"}}>عرض الكل</button>
                </div>
            ...
            <div style={{marginTop:44,padding:"22px 16px",borderRadius:18,background:"rgba(109,40,217,.07)",border:"1px solid rgba(109,40,217,.16)",display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,textAlign:"center",animation:"fadeUp .6s .2s both"}}>
              {[["1500+","سيارة","🚗"],["500+","وكالة","🏢"],["58","ولاية","📍"],["4.9★","تقييم","⭐"]].map(([n,l,ic])=>(
                <div key={l}><div style={{fontSize:18,fontWeight:900,color:"#C084FC"}}>{n}</div><div style={{fontSize:10,color:"rgba(255,255,255,.32)",marginTop:2}}>{l}</div><div style={{fontSize:16,marginTop:2}}>{ic}</div></div>
              ))}
            </div>
          </>
        }
