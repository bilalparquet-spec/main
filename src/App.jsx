...
export default function App() {
  ...
  // Sync auth state when localStorage changes (e.g., after login in another tab)
  useEffect(() => {
  ...
  if (authPage === "login")    return <LoginPage    onBack={()=>setAuthPage(null)} onGoRegister={()=>setAuthPage("register")} onGoGoogle={()=>setAuthPage("google")} onGoApple={()=>setAuthPage("apple")} onSuccess={()=>{setCurrentUser(getCurrentUser());setAuthPage(null);}}/>;
  if (authPage === "register") return <RegisterPage onBack={()=>setAuthPage(null)} onGoLogin={()=>setAuthPage("login")} onGoGoogle={()=>setAuthPage("google")} onGoApple={()=>setAuthPage("apple")} onSuccess={()=>{setCurrentUser(getCurrentUser());setAuthPage(null);}}/>;
  ...
  return (
    <div style={{minHeight:"100vh",background:"#07080F",color:"#F1F5F9",overscrollBehavior:"contain"}}>
      ...
      <nav style={{position:"sticky",top:0,zIndex:100,background:"rgba(7,8,15,.92)",backdropFilter:"blur(18px)",borderBottom:"1px solid rgba(255,255,255,.07)",padding:"0 clamp(12px,3vw,24px)",height:"var(--nav-height, 58px)",display:"flex",alignItems:"center",gap:"clamp(8px,2vw,16px)"}}>
        <div onClick={()=>{setPage("home");setSelCar(null);setSelAgency(null);}} style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",flexShrink:0}}>
          <img src={LOGO_IMG} alt="logo" style={{width:32,height:32,borderRadius:8,objectFit:"cover",flexShrink:0}}/>
          <span style={{fontWeight:900,fontSize:"clamp(15px,4vw,18px)",background:"linear-gradient(90deg,#A78BFA,#818CF8)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>Rent&درايف</span>
        </div>
        <div style={{flex:1}}/>
        <button onClick={()=>setAgencyRegister(true)} style={{
          ...
          <span style={{
            fontSize:"clamp(9px,2.2vw,11px)", fontWeight:800, color:"rgba(255,255,255,.55)",
      ...
      <div style={{maxWidth:"var(--app-max-width, 900px)",margin:"0 auto",padding:"clamp(14px,3vw,24px) var(--app-padding, 14px)"}}>
        {page==="car"&&selCar
          ...
          ? <MessagesPage/>
          : page==="profile"
          ? <ProfilePage onLogout={()=>{setCurrentUser(null);setPage("home");}}/>
          : <>
            {/* Hero */}
