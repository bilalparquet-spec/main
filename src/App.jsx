...
export default function App() {
  ...
  // ── Splash screen - show every time app loads ──────────────────────────────
  const [splashDone, setSplashDone] = useState(false);

  // ── Persist state ─────────────────────────────────────────────────────────
  const [onboarded, setOnboarded]     = useState(() => localStorage.getItem("onboarded") === "1");
  // Location state - NOT auto-requested, only on user action
  ...
  // ── Persist location state when granted ──────────────────────────────────
  useEffect(() => {
  ...
  // ── Trigger location request (lazy) ───────────────────────────────────────
  const triggerLocationRequest = () => {
    if (!locationDone && !locationRequested) {
      setLocationRequested(true);
      requestLocation();
    }
  };

  // ── Search focus handler (triggers location) ───────────────────────────────
  ...
  // 3. Location screen (only when explicitly triggered)
  if (locationRequested && !locationDone) {
    return (
      <LocationPermissionScreen
        status={locStatus}
        onGranted={() => requestLocation()}
        onManualSelect={(w) => { setUserWilaya(w); setLocationDone(true); setLocationRequested(false); localStorage.setItem("wilaya",w); localStorage.setItem("locDone","1"); }}
        onSkip={() => { setUserWilaya(null); setLocationDone(true); setLocationRequested(false); localStorage.setItem("locDone","1"); }}
      />
    );
  }
  ...
  return (
    <div style={{minHeight:"100vh",background:"#07080F",color:"#F1F5F9",overscrollBehavior:"contain",WebkitOverflowScrolling:"touch"}}>
      ...
      <nav style={{position:"sticky",top:0,zIndex:100,background:"rgba(7,8,15,.92)",backdropFilter:"blur(18px)",borderBottom:"1px solid rgba(255,255,255,.07)",padding:"0 clamp(12px,3vw,24px)",height:"var(--nav-height, 58px)",display:"flex",alignItems:"center",gap:"clamp(8px,2vw,16px)"}}>
        <div onClick={goHome} style={{display:"flex",alignItems:"center",gap:8,cursor:"pointer",flexShrink:0}}>
          {/* Real logo image */}
          <div style={{width:36,height:36,borderRadius:9,overflow:"hidden",flexShrink:0,boxShadow:"0 2px 12px rgba(124,58,237,.3)"}}>
            <img
              src={APP_LOGO_IMG}
              alt="logo"
        ...
        {[
          ...
          return (
            <button key={label} onClick={()=>{
              ...
              // Trigger location when clicking "رحلاتي"
              if (isTrips) {
                triggerLocationRequest();
              }
              navigate(path);
