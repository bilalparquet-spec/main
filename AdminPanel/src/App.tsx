...
export default function AdminPanel() {
  ...
  async function handleLogin() {
    ...
    try {
      ...
      if(!email.includes("@")) throw new Error("يرجى إدخال بريد إلكتروني صحيح");
      const {error: authError} = await supabase.auth.signInWithPassword({email, password: loginForm.password});
      if(authError) throw authError;
      const {data: profiles, error: profileError} = await supabase.from("profiles").select("*").eq("email", email).limit(1);
    ...
    } catch(e) { setLoginErr(e.message || "البريد الإلكتروني أو كلمة المرور غير صحيحة"); }
  }
    ...
    <div dir="rtl" style={{minHeight:"100vh",background:S.bg,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      ...
      <div style={{width:"100%",maxWidth:420,animation:"fadeIn .4s ease"}}>
        ...
        <div style={{background:S.bgCard,border:`1px solid ${S.border}`,borderRadius:20,padding:"32px 28px"}}>
          {[{label:"البريد الإلكتروني",key:"email",type:"email"},{label:"كلمة المرور",key:"password",type:"password"}].map(f=>(
...
function AddAgencyModal({onClose,setAgencies}) {
  ...
  async function handleAdd() {
    ...
    if(supabaseReady) { 
      try { await supabase.from("agencies").insert(row); } catch(e) { console.error(e); }
    }
...
function AdminsPage({admins,setAdmins,auth}) {
  ...
  const [newAdmin,setNewAdmin] = useState({name:"",email:"",password:"",role:"messages"});
  const [addErr,setAddErr]=useState("");

  async function handleAdd() {
    setAddErr("");
    if(!newAdmin.name||!newAdmin.email||!newAdmin.password) return;
    const email = String(newAdmin.email||"").trim().toLowerCase();
    if(!email.includes("@")) { setAddErr("يرجى إدخال بريد إلكتروني صحيح"); return; }
    if(admins.some(a=>String(a.email).toLowerCase()===email)) { setAddErr("البريد الإلكتروني محجوز"); return; }
    ...
    try {
      if(supabaseReady) {
        const {data: sign, error: signErr} = await supabase.auth.signUp({email, password:newAdmin.password, options:{data:{full_name:newAdmin.name,role}}});
        if(signErr) throw signErr;
        const id = sign?.user?.id || (crypto.randomUUID ? crypto.randomUUID() : "adm"+Date.now());
        const {error: insertErr} = await supabase.from("profiles").insert({id,email,full_name:newAdmin.name,role,permissions,status:"active"});
        if(insertErr) throw insertErr;
      }
      setAdmins(p=>[...p,{ id:"adm"+(Date.now()),name:newAdmin.name,email,password:"", role:newAdmin.role,avatar:newAdmin.name[0],permissions:newAdmin.role==="superadmin"?["all"]:[newAdmin.role], active:true,createdAt:new Date().toISOString().split("T")[0] }]);
      setShowAdd(false); setNewAdmin({name:"",email:"",password:"",role:"messages"});
      ...
      {showAdd&&(
        <div style={{background:S.bgCard,border:`1px solid ${S.purple}44`,borderRadius:14,padding:20,marginBottom:20}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:12,marginBottom:14}}>
            {[{k:"name",l:"الاسم الكامل"},{k:"email",l:"البريد الإلكتروني"},{k:"password",l:"كلمة المرور"}].map(f=>(
              <div key={f.k}>
                <label style={{fontSize:11,color:S.textDim,fontWeight:700,display:"block",marginBottom:5}}>{f.l}</label>
        ...
        {admins.map(admin=>(
          <div key={admin.id} style={{background:S.bgCard,border:`1px solid ${S.border}`,borderRadius:14,padding:18,display:"flex",alignItems:"center",gap:16}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:12,color:S.textMuted,fontFamily:"monospace"}}>{admin.email} · أضيف {formatDate(admin.createdAt)}</div>
            </div>
            <div style={{display:"flex",gap:8}}>
...
function UserAccountsPage({userRequests, setUserRequests, refreshUserRequests}) {
  ...
  const handleApprove=async (req)=>{
    if(supabaseReady) try { await supabase.from("pending_requests").update({status:"approved",reviewed_at:new Date().toISOString()}).eq("id",req.id); } catch(e) {}
  ...
  const handleReject=async (req)=>{
    if(supabaseReady) try { await supabase.from("pending_requests").update({status:"rejected",reviewed_at:new Date().toISOString()}).eq("id",req.id); } catch(e) {}
  ...
  const handleDelete=async (req)=>{
    if(supabaseReady) try { await supabase.from("pending_requests").delete().eq("id",req.id); } catch(e) {}
