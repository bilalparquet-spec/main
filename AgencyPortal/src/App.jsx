...
const AgencyLogo = ({agency, size=40, style={}}) => {
  ...
  const c = colors[Math.abs(String(agency.id||"").split("").reduce((a,ch)=>a+ch.charCodeAt(0),0)) % colors.length] || colors[0];
...
export default function App() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if(!email.includes("@")) throw new Error(lang==="ar"?"يرجى إدخال بريد إلكتروني صحيح":"Please enter a valid email");
      const {error: authError} = await supabase.auth.signInWithPassword({email, password: loginForm.password});
      if(authError) throw authError;
      let {data: profiles, error: pErr} = await supabase.from("profiles").select("*").eq("email", email).limit(1);
      if(pErr) throw pErr;
      const profile = profiles?.[0];
      if(!profile || !["agency","agency_owner"].includes(profile.role)) throw new Error(lang==="ar"?"هذا الحساب لا يملك صلاحية الوكالة":"This account is not an agency account");
      ...
      setLoggedIn(ag);
      setLoginErr("");
    } catch(e) { setLoginErr(e.message || (lang==="ar"?"البريد الإلكتروني أو كلمة المرور غير صحيحة":"Invalid email or password")); }
  };
  ...
  if (loggedIn) {
    return (
      <AgencyPortal
        ...
        onLogout={() => { setLoggedIn(null); setLoginForm({email:"",password:""}); }}
      />
    );
  }
