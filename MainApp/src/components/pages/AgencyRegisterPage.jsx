...
import { getAppLanguage, SUPABASE_URL, SUPABASE_ANON_KEY, SUPABASE_READY } from "../../lib/supabase.js";
import { IconBack, IconBuilding, IconSuccess, IconUserPlus, IconPhone, IconPin, IconLock, IconEye, IconEyeOff, IconDocument, IconChevronRight, IconShield } from "../ui/AppIcons.jsx";
...
export function AgencyRegisterPage({ onBack } = {}) {
  ...
  const canNext = () => {
    ...
    if (step === 1) return form.password.length >= 6 && form.email.trim() && form.phone.trim();
    ...
    try {
      ...
      const signUpRes = await fetch(`${SUPABASE_URL}/auth/v1/signup`, {
        ...
        body: JSON.stringify({
          ...
          data: { full_name: form.fullName, phone: form.phone, role: "agency_owner" }
        }),
      });
      const signUpData = await signUpRes.json().catch(() => ({}));
      if (!signUpRes.ok) throw new Error(signUpData?.error_description || signUpData?.msg || signUpData?.message || "فشل إنشاء الحساب");

      const userId = signUpData?.user?.id;
      if (!userId) throw new Error("لم يتم إنشاء الحساب بشكل صحيح");
      const token = signUpData?.session?.access_token || SUPABASE_ANON_KEY;

      // 2. إنشاء سجل في جدول agencies بحالة pending
      const agencyId = "ag" + Date.now();
      await fetch(`${SUPABASE_URL}/rest/v1/agencies`, {
        method: "POST",
      ...
      // 3. إنشاء profile بدور agency_owner و status pending
      ...
      setSubmitErr(e.message || "حدث خطأ، حاول مجدداً");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div dir={dir} style={{
      ...
      <div style={{ maxWidth: 580, margin: "0 auto", padding: "clamp(18px,4vw,28px) clamp(14px,4vw,24px) 200px" }}>
        {step === 0 && (
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 580, margin: "0 auto", display: "flex", gap: 12 }}>
          ...
          {step < STEPS.length - 1 ? (
            ...
            <div style={{flex:1}}>
              {submitErr && <div style={{background:"rgba(239,68,68,.1)",border:"1px solid rgba(239,68,68,.25)",borderRadius:12,padding:"9px 13px",fontSize:12,color:"#FCA5A5",marginBottom:10,textAlign:"center"}}>⚠️ {submitErr}</div>}
              <button onClick={() => canNext() && !submitting && submit()} className="btn-press" style={{
                width:"100%",
                padding: "14px 18px",
                borderRadius: 16,
                border: "none",
                background: canNext() && !submitting ? "linear-gradient(135deg,#059669,#34D399)" : "rgba(255,255,255,.07)",
                color: canNext() && !submitting ? "#fff" : "rgba(255,255,255,.30)",
                fontFamily: "inherit",
                fontWeight: 900,
                fontSize: 15,
                cursor: canNext() && !submitting ? "pointer" : "not-allowed",
                boxShadow: canNext() && !submitting ? "0 8px 28px rgba(52,211,153,.32)" : "none",
              }}>{submitting ? "جاري الإرسال..." : "إرسال الطلب"}</button>
            </div>
          )}
        </div>
      </div>
