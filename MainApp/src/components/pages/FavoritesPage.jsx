...
export function FavoritesPage({ liked, cars = [], onLike, onOpenCar, onRequireAuth, onSearch, isLoggedIn = false, appLang = "ar" }) {
  ...
  return (
    <div style={{ minHeight: "62vh", padding: "6px 0 102px", animation: "fadeUp .28s ease", color: "#F1F5F9" }}>
      <section style={{
        margin: "0 0 14px",
        borderRadius: 18,
        ...
        <div style={{ display: "flex", alignItems: "flex-start", gap: 9 }}>
          <div style={{ width: 30, height: 30, borderRadius: 12, flexShrink: 0, display: "grid", placeItems: "center", background: "rgba(239,68,68,.08)", border: "1px solid rgba(239,68,68,.18)" }}>
            <IconHeart active size={16} />
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <h2 style={{ margin: "0 0 3px", color: "#fff", fontSize: 14.5, fontWeight: 950, lineHeight: 1.35 }}>{t.title}</h2>
            <p style={{ margin: 0, color: "rgba(255,255,255,.58)", fontSize: 11.2, lineHeight: 1.65 }}>{t.subtitle}</p>
          </div>
        </div>
        <button
          ...
          style={{
            ...
            borderRadius: 14,
            minHeight: 40,
            background: isLoggedIn ? "rgba(124,58,237,.1)" : "linear-gradient(135deg,#7C3AED,#4F46E5)",
        ...
        >
          <IconSearchSm size={15} color={isLoggedIn ? "#C4B5FD" : "#fff"} />
          {t.action}
        </button>
      </section>
      ...
      {isLoggedIn && (
        <>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, margin: "8px 2px 12px" }}>
            <h3 style={{ margin: 0, color: "#fff", fontSize: 13.5, fontWeight: 950 }}>{heading}</h3>
            <span style={{ color: "rgba(255,255,255,.36)", fontSize: 10.5, fontWeight: 800 }}>{shownCars.length}</span>
          </div>
          {shownCars.length === 0 ? (
