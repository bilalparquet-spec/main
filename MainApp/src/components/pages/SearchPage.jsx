...
const wilayas = ["Alger", "Batna", "Oran", "Constantine", "Sétif", "Annaba", "Tlemcen", "Biskra", "Blida", "Tizi Ouzou"];
...
function SearchCarRow({ car, onOpenCar, index, lang }) {
  ...
  return (
    <button onClick={() => onOpenCar?.(car)} className="btn-press" style={{ width: "100%", border: "1px solid rgba(255,255,255,.08)", background: "rgba(255,255,255,.035)", borderRadius: 20, padding: 10, display: "flex", gap: 12, alignItems: "center", color: "#fff", cursor: "pointer", fontFamily: "inherit", textAlign: "start", animation: `fadeUp .28s ease ${Math.min(index, 6) * .04}s both` }}>
      <div style={{ width: 96, height: 74, borderRadius: 14, overflow: "hidden", flexShrink: 0, position: "relative", background: "#0B0C15" }}>
...
export function SearchPage({ cars = [], onBack, onOpenCar, appLang = "ar", userWilaya }) {
  ...
  const title = submitted || hasFilters ? t.results : t.allCars;
  ...
  return (
    <div style={{ minHeight: "72vh", padding: "4px 0 108px", animation: "pageSlideIn .25s ease", color: "#F1F5F9" }}>
      <section style={{ borderRadius: 28, background: "rgba(255,255,255,.055)", border: "1px solid rgba(255,255,255,.08)", boxShadow: "0 24px 90px rgba(0,0,0,.34)", padding: "18px 18px 20px", marginBottom: 22 }}>
        <button onClick={onBack} aria-label="back" style={{ width: 40, height: 40, borderRadius: 16, background: "transparent", border: "none", color: "#fff", display: "grid", placeItems: "center", cursor: "pointer", marginBottom: 8 }}>
          <IconBack size={24} color="rgba(255,255,255,.78)" />
        </button>

        <label style={{ display: "block", color: "rgba(255,255,255,.86)", fontSize: 16, fontWeight: 850, marginBottom: 7 }}>{t.where}</label>
        <div style={{ position: "relative", marginBottom: 12 }}>
          <IconSearchSm size={17} color="rgba(255,255,255,.34)" style={{ position: "absolute", top: "50%", insetInlineStart: 15, transform: "translateY(-50%)" }} />
          <input value={where} onChange={(e) => { setWhere(e.target.value); setShowSuggestions(true); }} onFocus={() => setShowSuggestions(true)} placeholder={t.wherePlaceholder} style={{ width: "100%", height: 58, borderRadius: 15, border: "1.5px solid rgba(255,255,255,.14)", background: "#03040A", color: "#fff", outline: "none", padding: "0 44px", fontSize: 16, fontWeight: 700, fontFamily: "inherit" }} />
          {where && <button onClick={() => setWhere("")} style={{ position: "absolute", top: "50%", insetInlineEnd: 10, transform: "translateY(-50%)", width: 34, height: 34, borderRadius: 12, border: "none", background: "rgba(255,255,255,.05)", display: "grid", placeItems: "center", cursor: "pointer" }}><IconClose size={15} color="rgba(255,255,255,.5)" /></button>}
        </div>

        {showSuggestions && (
          <div style={{ display: "flex", gap: 8, overflowX: "auto", padding: "0 0 12px", scrollbarWidth: "none" }}>
            {[t.nearMe, ...wilayas].map((w) => (
              <button key={w} onClick={() => { setWhere(w === t.nearMe ? (userWilaya || "") : w); setShowSuggestions(false); }} style={{ flexShrink: 0, border: "1px solid rgba(167,139,250,.22)", borderRadius: 999, background: "rgba(124,58,237,.1)", color: "#DDD6FE", padding: "8px 12px", fontFamily: "inherit", fontSize: 12, fontWeight: 900, cursor: "pointer" }}>{w}</button>
            ))}
          </div>
        )}

        <label style={{ display: "block", color: "rgba(255,255,255,.86)", fontSize: 16, fontWeight: 850, marginBottom: 7 }}>{t.when}</label>
        <div style={{ position: "relative", marginBottom: 13 }}>
          <IconCalendar size={17} color="rgba(255,255,255,.34)" style={{ position: "absolute", top: "50%", insetInlineStart: 15, transform: "translateY(-50%)" }} />
          <input value={when} onChange={(e) => setWhen(e.target.value)} onFocus={() => setShowDates(true)} placeholder={t.whenPlaceholder} style={{ width: "100%", height: 58, borderRadius: 15, border: "1.5px solid rgba(255,255,255,.14)", background: "#03040A", color: "#fff", outline: "none", padding: "0 44px", fontSize: 16, fontWeight: 700, fontFamily: "inherit" }} />
          {when && <button onClick={() => setWhen("")} style={{ position: "absolute", top: "50%", insetInlineEnd: 10, transform: "translateY(-50%)", width: 34, height: 34, borderRadius: 12, border: "none", background: "rgba(255,255,255,.05)", display: "grid", placeItems: "center", cursor: "pointer" }}><IconClose size={15} color="rgba(255,255,255,.5)" /></button>}
        </div>

        {showDates && (
          <div style={{ display: "flex", gap: 8, overflowX: "auto", padding: "0 0 14px", scrollbarWidth: "none" }}>
            {dateChips.map((item) => (
              <button key={item} onClick={() => { setWhen(item); setShowDates(false); }} style={{ flexShrink: 0, border: "1px solid rgba(255,255,255,.12)", borderRadius: 999, background: "rgba(255,255,255,.05)", color: "rgba(255,255,255,.82)", padding: "8px 12px", fontFamily: "inherit", fontSize: 12, fontWeight: 900, cursor: "pointer" }}>{item}</button>
            ))}
          </div>
        )}

        <button onClick={submitSearch} className="btn-press" style={{ width: "100%", minHeight: 58, border: "none", borderRadius: 16, background: "linear-gradient(135deg,#7C3AED,#4F46E5)", color: "#fff", fontFamily: "inherit", fontSize: 17, fontWeight: 950, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 9 }}>
          <IconSearchSm size={18} color="#fff" />
          {t.search}
        </button>
        {hasFilters && <button onClick={clearSearch} style={{ width: "100%", marginTop: 9, minHeight: 42, border: "1px solid rgba(255,255,255,.1)", borderRadius: 14, background: "rgba(255,255,255,.035)", color: "rgba(255,255,255,.62)", fontFamily: "inherit", fontSize: 12.5, fontWeight: 900, cursor: "pointer" }}>{t.clear}</button>}
      </section>

      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, margin: "0 2px 13px" }}>
        <div>
          <h2 style={{ margin: 0, color: "#fff", fontSize: 18, fontWeight: 950 }}>{title}</h2>
          <div style={{ marginTop: 3, color: "rgba(255,255,255,.36)", fontSize: 12 }}>{results.length} {t.cars}</div>
        </div>
        <button onClick={() => setShowSuggestions(true)} style={{ width: 42, height: 42, borderRadius: 15, border: "1px solid rgba(255,255,255,.1)", background: "rgba(255,255,255,.04)", display: "grid", placeItems: "center", cursor: "pointer" }}>
          <IconFilter size={18} color="rgba(255,255,255,.66)" />
        </button>
      </div>

      {results.length === 0 ? (
        <div style={{ textAlign: "start", padding: "16px 0 0" }}>
          <NoCarsIllustration />
          <h2 style={{ color: "rgba(255,255,255,.52)", fontSize: 28, fontWeight: 950, margin: "0 0 8px" }}>{t.noCars}</h2>
          <p style={{ color: "rgba(255,255,255,.36)", fontSize: 15, lineHeight: 1.8, margin: 0 }}>{t.noCarsSub}</p>
        </div>
      ) : (
