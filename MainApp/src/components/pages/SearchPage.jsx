...
import { IconBack, IconSearchSm, IconClose, IconPin, IconCalendar } from "../ui/AppIcons.jsx";
...
export function SearchPage({ cars = [], onBack, onOpenCar, appLang = "ar", userWilaya }) {
  ...
  const canShowResults = Boolean(submitted || hasFilters);
  ...
  return (
    <div style={{ minHeight: "72vh", padding: "0 0 108px", animation: "pageSlideIn .25s ease", color: "#F1F5F9" }}>
      <section style={{ borderRadius: 24, background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.08)", boxShadow: "0 20px 70px rgba(0,0,0,.3)", padding: "14px 14px 16px", margin: "0 0 18px" }}>
        <button onClick={onBack} aria-label="back" style={{ width: 34, height: 34, borderRadius: 14, background: "transparent", border: "none", color: "#fff", display: "grid", placeItems: "center", cursor: "pointer", marginBottom: 6 }}>
          <IconBack size={21} color="rgba(255,255,255,.78)" />
        </button>

        <label style={{ display: "block", color: "rgba(255,255,255,.86)", fontSize: 14, fontWeight: 850, marginBottom: 5 }}>{t.where}</label>
        <div style={{ position: "relative", marginBottom: 10 }}>
          <IconSearchSm size={16} color="rgba(255,255,255,.34)" style={{ position: "absolute", top: "50%", insetInlineStart: 14, transform: "translateY(-50%)" }} />
          <input value={where} onChange={(e) => setWhere(e.target.value)} placeholder={t.wherePlaceholder} style={{ width: "100%", height: 50, borderRadius: 14, border: "1.5px solid rgba(255,255,255,.14)", background: "#03040A", color: "#fff", outline: "none", padding: "0 42px", fontSize: 14, fontWeight: 700, fontFamily: "inherit" }} />
          {where && <button onClick={() => setWhere("")} style={{ position: "absolute", top: "50%", insetInlineEnd: 10, transform: "translateY(-50%)", width: 32, height: 32, borderRadius: 12, border: "none", background: "rgba(255,255,255,.05)", display: "grid", placeItems: "center", cursor: "pointer" }}><IconClose size={14} color="rgba(255,255,255,.5)" /></button>}
        </div>

        <label style={{ display: "block", color: "rgba(255,255,255,.86)", fontSize: 14, fontWeight: 850, marginBottom: 5 }}>{t.when}</label>
        <div style={{ position: "relative", marginBottom: 12 }}>
          <IconCalendar size={16} color="rgba(255,255,255,.34)" style={{ position: "absolute", top: "50%", insetInlineStart: 14, transform: "translateY(-50%)" }} />
          <input value={when} onChange={(e) => setWhen(e.target.value)} placeholder={t.whenPlaceholder} style={{ width: "100%", height: 50, borderRadius: 14, border: "1.5px solid rgba(255,255,255,.14)", background: "#03040A", color: "#fff", outline: "none", padding: "0 42px", fontSize: 14, fontWeight: 700, fontFamily: "inherit" }} />
          {when && <button onClick={() => setWhen("")} style={{ position: "absolute", top: "50%", insetInlineEnd: 9, transform: "translateY(-50%)", width: 32, height: 32, borderRadius: 12, border: "none", background: "rgba(255,255,255,.05)", display: "grid", placeItems: "center", cursor: "pointer" }}><IconClose size={14} color="rgba(255,255,255,.5)" /></button>}
        </div>

        <button onClick={submitSearch} className="btn-press" style={{ width: "100%", minHeight: 52, border: "none", borderRadius: 15, background: "linear-gradient(135deg,#7C3AED,#4F46E5)", color: "#fff", fontFamily: "inherit", fontSize: 15, fontWeight: 950, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
      ...
      {visibleResults.length === 0 ? (
        <div style={{ textAlign: "start", padding: "16px 0 0" }}>
          <NoCarsIllustration />
          <h2 style={{ color: "rgba(255,255,255,.52)", fontSize: 24, fontWeight: 950, margin: "0 0 7px" }}>{t.noCars}</h2>
          <p style={{ color: "rgba(255,255,255,.36)", fontSize: 13.5, lineHeight: 1.75, margin: 0 }}>{t.noCarsSub}</p>
        </div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {visibleResults.map((car, i) => <SearchCarRow key={car.id || i} car={car} index={i} onOpenCar={onOpenCar} lang={appLang} />)}
        </div>
      )}
    </div>
  );
