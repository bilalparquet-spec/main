  ...
  return (
    <div className="tiny-ui" style={{ minHeight: "calc(100vh - 170px)", padding: "4px 0 104px", animation: "fadeUp .28s ease", background: "#050506", color: "#fff" }}>
      <header style={{ textAlign: "center", padding: "6px 0 12px" }}>
        <h1 style={{ margin: 0, fontSize: 18, fontWeight: 950, letterSpacing: ".2px" }}>علبة الرسائل</h1>
      </header>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", borderBottom: "1px solid rgba(255,255,255,.12)", margin: "0 calc(var(--app-padding, 14px) * -1) 0" }}>
        <TabButton active={tab === "messages"} onClick={() => setTab("messages")} label="الرسائل" icon={<IconBubble size={13} color="currentColor" />} />
        <TabButton active={tab === "notifications"} onClick={() => setTab("notifications")} label="الإشعارات" icon={<IconBell size={13} color="currentColor" />} />
      </div>

      {tab === "notifications" ? (
        <section style={{ padding: "14px 0 0" }}>
          <div style={{ textAlign: "center", borderTop: "1px solid rgba(255,255,255,.12)", borderBottom: "1px solid rgba(255,255,255,.12)", padding: "10px 0", marginBottom: 16, color: "rgba(255,255,255,.88)", fontSize: 11, fontWeight: 950, letterSpacing: ".8px" }}>نشاط</div>
          <NotificationCard />
