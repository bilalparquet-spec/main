...
export default function App() {
  ...
  const tx = {
    ar: { explore:"استكشف", searchTab:"بحث", favorites:"المفضلة", trips:"رحلاتي", messages:"رسائل", profile:"أكثر", loginRequired:"سجّل الدخول للمتابعة", loginSub:"هذه الصفحة مخصصة للمستخدمين المسجلين.", login:"تسجيل الدخول", register:"إنشاء حساب", nearby:"قريب مني", suggest:"اقترح لي سيارة", search:"ابحث: سيارة، ولاية، نوع..." },
    fr: { explore:"Explorer", searchTab:"Recherche", favorites:"Favoris", trips:"Trajets", messages:"Messages", profile:"Plus", loginRequired:"Connectez-vous pour continuer", loginSub:"Cette page est réservée aux utilisateurs connectés.", login:"Connexion", register:"Créer un compte", nearby:"Près de moi", suggest:"Suggérer une voiture", search:"Rechercher: voiture, wilaya, type..." },
    en: { explore:"Explore", searchTab:"Search", favorites:"Favorites", trips:"Trips", messages:"Messages", profile:"More", loginRequired:"Sign in to continue", loginSub:"This page is for signed-in users.", login:"Sign in", register:"Create account", nearby:"Near me", suggest:"Suggest a car", search:"Search: car, wilaya, type..." }
  ...
  // ── Keep guest users out of protected tab routes ──────────────────────────
  useEffect(() => {
    if (!isLoggedIn && pathname === "/favorites") {
      setAuthSheet(true);
  ...
  const renderPage = () => {
    ...
    if (pathname.startsWith("/agency/") && selAgency)
      return <AgencyDetail agency={selAgency} cars={sbCars} onBack={()=>{ setSelAgency(null); navigate(-1); }} onOpenCar={openCar} liked={liked} onLike={toggleLike}/>;

    if (pathname === "/search")
      return <SearchPage cars={sbCars} onBack={()=>navigate("/")} onOpenCar={openCar} appLang={appLang} userWilaya={userWilaya}/>;
    ...
    if ((["/trips", "/messages"].includes(pathname) || pathname.startsWith("/profile")) && !isLoggedIn) {
      return (
    ...
    if (pathname === "/favorites")
      return <FavoritesPage liked={liked} cars={sbCars} onLike={toggleLike} onOpenCar={openCar} isLoggedIn={isLoggedIn} appLang={appLang} onRequireAuth={()=>setAuthSheet(true)} onSearch={()=>navigate("/search")}/>;
  ...
  return (
    <div dir={appDir} data-app-lang={appLang} style={{minHeight:"100vh",background:"#07080F",color:"#F1F5F9",overscrollBehavior:"contain"}}>
      ...
      {!isFullDetailRoute && <div className="bottom-nav" dir={appDir} data-nav-dir={appDir} style={{position:"fixed",bottom:0,left:0,right:0,background:"rgba(7,8,15,.96)",backdropFilter:"blur(20px)",borderTop:"1px solid rgba(255,255,255,.07)",display:"flex",flexDirection:"row",direction:appDir,justifyContent:"space-around",padding:"10px 0 max(10px,env(safe-area-inset-bottom))",zIndex:99}}>
        {[
          { label:tx.searchTab || tx.explore, Icon:IconSearch, path:"/search"    },
          ...
          const isHome = path === "/search";
          const active = isHome
            ? (pathname === "/" || pathname === "/search" || pathname.startsWith("/car/") || pathname.startsWith("/agency/"))
            : pathname === path;
